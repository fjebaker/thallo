...
def pretty_print_events(events: list[Event]):
    ...
    for event in events:
        ...
        print(INDENT + Style.DIM + BOX_TOP_LEFT, start_date + Style.RESET_ALL, TIME_FMT + start_time + TIME_END, end = "")
        print(" - ", end = "")
        print(Style.DIM + end_date + Style.RESET_ALL,  TIME_FMT + end_time + TIME_END)
        print(INDENT + Style.DIM + BOX_BOT_LEFT + Style.RESET_ALL,  Style.BRIGHT + event["name"] + Style.RESET_ALL, end="")
